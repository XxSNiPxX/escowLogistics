...
func TestSystemInfo4Query(t *testing.T) {
	keeper, ctx := keepertest.LogisticsbetaKeeper(t)
	item := createTestSystemInfo4(keeper, ctx)
	tests := []struct {
		desc     string
		request  *types.QueryGetSystemInfo4Request
		response *types.QueryGetSystemInfo4Response
		err      error
	}{
		{
			desc:     "First",
			request:  &types.QueryGetSystemInfo4Request{},
			response: &types.QueryGetSystemInfo4Response{SystemInfo4: item},
		},
		{
			desc: "InvalidRequest",
	...
	for _, tc := range tests {
		t.Run(tc.desc, func(t *testing.T) {
			response, err := keeper.SystemInfo4(ctx, tc.request)
			if tc.err != nil {
